...
class Ant(Insect):
    ...
    def add_to(self, place):
        if place.ant is None:
            ...
            if current_ant.can_contain(self):
                current_ant.contain_ant(self)
            elif self.can_contain(current_ant):
                self.contain_ant(current_ant)
                place.ant=self
            else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        p=self.place
        first_step=0
        while first_step<self.min_range:
            if p!=beehive:
                p=p.entrance
                first_step +=1
            else:
                return None
        step = self.min_range
        while p != beehive and step<= self.max_range:
            if p.bees !=[]:
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        death=self.armor<=amount
        def reflect_damage(amount_damage):
            for bee in self.place.bees.copy():
                Insect.reduce_armor(bee,amount_damage)
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        if self.digesting ==0:
            if self.place.bees !=[]:
                bee = rANTdom_else_none(self.place.bees.copy())
                print("DEBUG:killed_bee",bee.__repr__())
                self.eat_bee(bee)
            else:
                return None
        else:
            self.digesting -=1
        print("DEBUG:bees_now",self.place.bees.__len__())
        # END Problem 6
...
# END Problem 13
    ...
    def action(self, gamestate):
        ...
        if not self.true_queen :
            ...
            p=self.place.exit
            while p != None:
                if p.ant != None and not p.ant.encouraged:
                    p.ant.damage *= 2
                    p.ant.encouraged =True
                if isinstance(p.ant,ContainerAnt) and p.ant.contained_ant !=None and not  p.ant.contained_ant.encouraged :
                    p.ant.contained_ant.damage *=2
                    p.ant.contained_ant.encouraged=True
                p=p.exit 
        # END Problem 13
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        return True if self.place.ant is not None and self.place.ant.blocks_path == True else False
        # END Problem 7
