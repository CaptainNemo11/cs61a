...
def totals_tree(m):
    ...
    return tree(total_weight(l)+total_weight(r),\
            [totals_tree(l),totals_tree(r)])
...
def preorder(t):
    ...
    if is_leaf(t):
        ...
        b_preorder=[]
        for b in branches(t):
            b_preorder += preorder(b)
        return [label(t)]+ b_preorder
    """
...
def quadratic(x, a, b, c):
    ...
    lower=lower_bound(x)
    upper=upper_bound(x)
    f_lower=a*lower*lower + b*lower + c
    f_upper=a*upper*upper + b*upper + c
    f_extreme=a*extreme*extreme + b*extreme + c
    if not lower<extreme<upper:
...
def check_par():
    ...
    True
    """
    r1 = interval(1, 11) # Replace this line!
