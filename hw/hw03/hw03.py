...
def g_iter(n):
    ...
    if n<=3:
        return n
    i=4
    curr,back_1,back_2=3,2,1
    while i<=n:
        curr,back_1,back_2=curr+2*back_1+3*back_2,curr,back_1
        i=i+1
    return curr
...
def missing_digits(n):
    ...
    if n<10:
        ...
        return n%10!=n//10 and n%10-n//10-1
...
def count_change(total):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(max_cent=2**floor(sqrt(total)),n=total):
        if max_cent==1:
            return 1
        if n==0:
            return 1
        if n<0:
            return 0 
        return helper(max_cent,n-max_cent)+helper(max_cent//2,n) 
    return helper()
...
def make_anonymous_factorial():
    ...
    True
    """
    return    (lambda f:lambda k:f(k,f))(lambda n,self:1 if n==1 else mul(n,self(sub(n,1),self)))
