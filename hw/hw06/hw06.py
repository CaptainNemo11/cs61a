...
class Coin:
    ...
    def worth(self):
        ...
        return self.cents+(age>0 and age-50)
...
def is_bst(t):
    ...
    branches=t.branches
    if len(branches)==1:
         if  sub_most_child(branches[0],'right')<=t.label or sub_most_child(branches[0],'left')>t.label:
             return is_bst(branches[0])

    elif len(branches)==2:
         if branches[0].label<=t.label<branches[1].label \
                 and sub_most_child(branches[0],'right')<=t.label<sub_most_child(branches[1],'left'):
             return is_bst(branches[0]) and is_bst(branches[1]) 
         else:
             return False
    else :
        return False
    ...
    if len(branch)==1:
        ...
        if max_bst(branch[0])<=t.label<min_bst(branch[1]):
...
def remove_all(link , value):
    ...
    """
    alias=link
    while alias.rest != Link.empty:
        if alias.rest.first==value:
            alias.rest=alias.rest.rest
        else:
            alias=alias.rest
...
def deep_map(f, link):
    ...
    if type(link.first)==Link:
        ret=Link(deep_map(f,link.first))
    else:
        ret=Link(f(link.first))
    ret.rest=deep_map(f,link.rest)
    return ret
    """
