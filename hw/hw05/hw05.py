...
def make_withdraw(balance, password):
    ...
    def withdraw(amount,passwd):
        nonlocal balance
        if passwd==password and len(wrong_passwd)<3:
            if balance<amount:
                return "Insufficient funds"
            else:
                balance -=amount
                return balance
        elif len(wrong_passwd)<3:
            wrong_passwd.append(passwd)
            return "Incorrect password"
        else:
            return "Too many incorrect attempts. Attempts: "+str(wrong_passwd)
    return withdraw
...
def make_joint(withdraw, old_pass, new_pass):
    ...
    """
    "*** YOUR CODE HERE ***"
    string= withdraw(0,old_pass)
    if type(string)==str:
        return string 
    def join_user(amount,join_pass):
        if join_pass == new_pass :
            return withdraw(amount,old_pass)
        else:
            return withdraw(amount,join_pass)
    return join_user
