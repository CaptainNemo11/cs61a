...
def trade(first, second):
    ...
    while not equal_prefix() and( m<len(first)-1 or n < len(second)-1):
...
def make_glookup(class_assignments):
    ...
    score_for_now=0
    total_for_now=0
    already_included={key : 0 for key in class_assignments.keys()}

    def glookup(keyword,score):
        nonlocal score_for_now,total_for_now
        if  not already_included[keyword]:
            total_for_now += class_assignments[keyword]
            already_included[keyword]=1
        score_for_now += score
        return score_for_now/total_for_now
    return glookup
...
def make_advanced_counter_maker():
    ...
    def make_counter():
        ...
        def dispatch(message):
            nonlocal user_count,global_count
            "*** YOUR CODE HERE ***"
            if message=='count':
                user_count +=1
                return user_count
